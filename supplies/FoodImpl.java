...
public abstract class FoodImpl extends SuppliesImpl implements Food {
	...
	/* Constructors */
	protected FoodImpl(int amount, int unit_weight, String name, int days,
			int fill) {
		super(amount, unit_weight, name);
		// Validate input
		if (days <= 0)
			throw new IllegalArgumentException(
					"Days till expiration must be positive!");
	...
	public String toString() {
		return super.toString() + " (expires in " + getDaysTillExpiration()
				+ " days)";
	}
}